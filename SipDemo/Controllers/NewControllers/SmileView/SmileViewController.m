...
@synthesize tagId,arrFeedList,objStatusFeedDetailViewController,objFormFeedDetailViewController,objFeedDetailViewController,popoverController,selectedIndxpath,objRouteFeedDetailViewController,objRecieptFeedDetailViewController;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateSmileNotification:) name:AC_USER_UNREAD_NOTIFICATION_UPDATE object:nil];
...
-(void)updateSmileNotification:(NSNotification*) note
{
    if ([checkPN isEqualToString:@"YES"]) {
        ...
        if ([nType isEqualToString:@"happyFace"]) {
...
-(void)requestForGetTagFeeds
{
    if([ConfigManager isInternetAvailable]){
        ...
        [DSBezelActivityView newActivityViewForView:sharedAppDelegate.window withLabel:@"Smile list..." width:180];
        
        if ([sharedAppDelegate.strCheckUserAndTag isEqualToString:@"User"]) {
            
            [[AmityCareServices sharedService] SmileFeedListInvocation:sharedAppDelegate.userObj.userId tagId:self.tagId status:@"1" delegate:self];
        }
        else
        {
            /*if (IS_DEVICE_IPAD) {
            ...
            if ([sharedAppDelegate.userObj.role isEqualToString:@"6"] || [sharedAppDelegate.userObj.role isEqualToString:@"8"] ) {
                ...
                [[AmityCareServices sharedService] AllHappyFacePostsInvocation:dic delegate:self];
                //}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    ACAlertView* deleteAlert = [[ACAlertView alloc] initWithTitle:nil message:@"Remove from happy face post?" delegate:self cancelButtonTitle:nil otherButtonTitles:@"YES",@"NO", nil];
