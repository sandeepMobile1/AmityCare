//
//  RecieptFeedDetailViewController.m
//  SipDemo
//
...
@interface RecieptFeedDetailViewController ()

@end

@implementation RecieptFeedDetailViewController

@synthesize feedDetails,checkBSAndFamily,locationVC,imgPhoto;
...
-(void)setFeedDetailsValues
{
    ...
    txtDescription.text=[self.feedDetails.recieptDescription capitalizedString];
    lblMerchant.text=[NSString stringWithFormat:@"Merchant Name: %@",[feedDetails.recieptMerchantName capitalizedString]];
    
    lblRecieptDate.text=@"";
    
    if (![self.feedDetails.recieptDate isEqualToString:@""]) {
        
        NSDateFormatter *df = [[NSDateFormatter alloc] init];
        [df setLocale:[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"]];
        [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
        
        NSDate *tdate = [df dateFromString:self.feedDetails.recieptDate];

        lblRecieptDate.text=[self shortStyleDate:tdate];

    }
    
    
    NSNumber *totalNumber=[NSNumber numberWithFloat: [self.feedDetails.recieptAmount floatValue]]; // The old syntax
    
    NSNumberFormatter *formatter = [[NSNumberFormatter alloc] init];
    [formatter setNumberStyle:NSNumberFormatterCurrencyStyle];
    NSString *strTotal = [formatter stringFromNumber:totalNumber];
    
    [lblAmount setText:[NSString stringWithFormat:@"Amount: %@",strTotal]];


    if ([self.feedDetails.recieptReimbursementStatus isEqualToString:@"1"]) {
        
        lblReimbursement.text=@"Reimbursement: Yes";
    }
    else if ([self.feedDetails.recieptReimbursementStatus isEqualToString:@"0"]) {
        
        lblReimbursement.text=@"Reimbursement: No";
    }
    else
    {
        lblReimbursement.text=@"";
    ...
    if ([sharedAppDelegate.userObj.role isEqualToString:@"3"]|| [sharedAppDelegate.userObj.role isEqualToString:@"5"])
    {
        ...
        NSLog(@"postimage= %@",[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",recieptImageURL,self.feedDetails.recieptImage]]);
        
        
        [imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",recieptImageURL,self.feedDetails.recieptImage]] placeholderImage:[UIImage imageNamed:@"user_default.png"]];
...
-(void)setFrame
{
    ...
    int y=txtDescription.frame.origin.y+5;
    
        CGSize sizeToFit;
    
            
            CGRect textRect = [self.feedDetails.recieptDescription boundingRectWithSize:CGSizeMake(txtDescription.frame.size.width, CGFLOAT_MAX)
                                                                      options:NSStringDrawingUsesLineFragmentOrigin
                                                                   attributes:@{NSFontAttributeName:txtDescription.font}
                                                                      context:nil];
            
            sizeToFit = textRect.size;
            
        [txtDescription setFrame:CGRectMake(txtDescription.frame.origin.x, txtDescription.frame.origin.y, txtDescription.frame.size.width, sizeToFit.height+5)];
        
        y=CGRectGetMaxY(txtDescription.frame)+10;
...
-(IBAction)btnRecieptImageAction:(id)sender
{
    ...
    {
        [sharedAppDelegate.window addSubview:largeView];

    }
    
    [imgLargeView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",recieptImageURL,self.feedDetails.recieptImage]] placeholderImage:[UIImage imageNamed:@"user_default.png"]];
...
-(void)secureCredentialDidSubmitted:(PasswordView *)view
{
    ...
    else
    {
        ...
        if([feedDetails.postType isEqualToString:@"1"]){
            self.imgPhoto.imageURL = feedDetails.postThumbnailURL;
            ...
            self.imgPhoto.videoURL = feedDetails.postVideoURL;
        }
...
-(void)secureCredentialDidSubmitted_Iphone:(PasswordViewIphone*)view
{
    ...
    else
    {
        ...
        if([self.feedDetails.postType isEqualToString:@"1"]){
            self.imgPhoto.imageURL = self.feedDetails.postThumbnailURL;
            self.imgPhoto.docType = documentTypeImage;
        }
        else if([self.feedDetails.postType isEqualToString:@"2"]){
            
            NSLog(@"URL %@",self.feedDetails.postVideoURL);
            
            self.imgPhoto.docType = documentTypeVideo;
            self.imgPhoto.videoURL = self.feedDetails.postVideoURL;
        }
        else if ([self.feedDetails.postType isEqualToString:@"3"]){
            self.imgPhoto.docType = documentTypeFiles;
            self.imgPhoto.documentURL = [NSString stringWithFormat:@"%@%@",smallPostThumbnailImageURL,self.feedDetails.postThumbnailURL];
        }
