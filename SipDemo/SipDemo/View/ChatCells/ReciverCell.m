...
{
    CGSize constainedSize = CGSizeMake(MAX_WIDTH, MAX_HEIGHT);
    CGSize expectedSize = [string sizeWithFont:[UIFont fontWithName:label.font.fontName size:label.font.pointSize ] constrainedToSize:constainedSize lineBreakMode:NSLineBreakByWordWrapping];
    return expectedSize;
}*/
...
-(void)layoutSubviews
{
    ...
    if (IS_DEVICE_IPAD) {
        ...
        CGSize namesize = CGSizeMake(45, 21);//[self constrainedWidth:_message.sender_uname label:lblUsername];
        ...
        frame.origin.x = self.bounds.size.width-15-mainView.frame.size.width;
        [mainView setFrame:frame];
        ...
        frame = lblTime.frame;
        frame.origin.x  = _width - lblTime.frame.size.width;
        frame.size.width = 75;
    ...
    {
        //CGSize namesize = CGSizeMake(45, 21);
        CGSize msgSize = [self constrainedWidth:_message.msg_text label:lblMessage];
...
{
   /* CGSize sizeToFit = [message.msg_text sizeWithFont:[UIFont fontWithName:appfontName size:14.0] constrainedToSize:CGSizeMake(292.0f, CGFLOAT_MAX) lineBreakMode:NSLineBreakByWordWrapping];
    NSLog(@"cell height =%f",fmaxf(52.0f, (float)sizeToFit.height + 26+10));
    return fmaxf(57.0f, (float)sizeToFit.height + 26+10+5);*/
    CGSize sizeToFit;
